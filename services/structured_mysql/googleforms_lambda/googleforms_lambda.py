    ...
    try: 
        for key in list(forms):
            ...
            if result is None:
                session.add(Form(
                    ...
                    created_at=forms[key]['created_at'], 
                    owner=forms[key]['owner'], 
                    published_url=forms[key]['published_url']))

        for key in range(len(questions)):
            result = session.query(Question).filter(
                    Question.form_question_id == questions[key]['form_question_id'],
                    Question.form_id == questions[key]['form_id']).order_by(Question.version.desc()).first()
            if result is None:
                session.add(Question(
                    unique_id = questions[key]['unique_id'], 
                    form_id = questions[key]['form_id'], 
                    text_question=questions[key]['text_question'], 
                    type=questions[key]['question_type'], 
                    version= 1, 
                    form_question_id = questions[key]['form_question_id']))
                session.commit()
                continue
        ...
        for key in range(len(answers)):
            ...
            if result is None:
                ...
                session.add(Answer(
                    unique_id = answers[key]['unique_id'], 
                    unique_question_id = answers[key]['unique_question_id'], 
                    response_id = answers[key]['response_id'], \
                    text_answer=answers[key]['text_answer'], 
                    timestamp=answers[key]['timestamp'], 
                    version=answers[key]['version'], 
                    form_id=answers[key]['form_id']))
                session.commit()
                continue
