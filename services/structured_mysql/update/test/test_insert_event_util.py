...
class TestInsertEventUtil:
    ...
    def test_get_google_events(self, event, google_events):
        assert google_events == get_google_events(event)
    ...
    def test_append_insert_or_update_queries_from_event(self, monkeypatch, db_connection, google_events):
        ...
        test = [
            ...
            '0, "NULL", "knowit.no_63rtu1seerufqsdhc4avduoggk@group.calendar.google.com", 1575381600, 1575396000, '
    ...
    def test_append_delete_queries_from_event(self, monkeypatch, db_connection, db_events):
        ...
        real = event_queries.get_sql_queries()
        test = ["DELETE FROM EventRatingType WHERE event_id = '3mbuq0b4pciqef8qghu4ouc715';"]

        assert real == test

    def test_append_single_delete_query_from_event(self, db_connection, db_events, google_events):
        ...
        real = event_queries.get_sql_queries()
        test = ["DELETE FROM EventRatingType WHERE id = '3mbuq0b4pciqef8qghu4ouc715';"]

        assert real == test

    def test_is_event_button_still_in_google_event(self, db_events, google_events):
        db_event = db_events[0]
